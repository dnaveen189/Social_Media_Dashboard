...
const Messages = () => {
  ...
  // Separate effect for fetching conversations
  useEffect(() => {
    if (token) {
      // eslint-disable-next-line react-hooks/exhaustive-deps
      fetchConversations();
    }
  }, [token]); // Remove fetchConversations from deps to avoid the warning
  ...
  // Separate effect for fetching messages when activeChat changes
  useEffect(() => {
    if (activeChat?._id && token) {
      // eslint-disable-next-line react-hooks/exhaustive-deps
      fetchMessages(activeChat._id);
    }
  }, [activeChat?._id, token]); // Remove fetchMessages from deps to avoid the warning
        ...
        <div className="flex-1 overflow-y-auto">
          {conversations.length > 0 ? (
            conversations.map((conv, index) => {
              // Create a stable key using user ID or index as fallback
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {messages.length > 0 ? (
                messages.map((msg, index) => {
                  // Create a stable key for messages
